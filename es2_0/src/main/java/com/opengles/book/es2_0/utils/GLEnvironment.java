...
/**
 * 一个去除了surface类，类似于glSurface类，我们可以使用GLEnvironment来进行渲染，并自由的指定渲染载体，
 * 可以是SurfaceView/TextureView或Pbuffer，也可以是Pixmap
 *
...
/*使用SurfaceView或者TextureView,customWindow可以为SurfaceTexture\SurfaceHolder或者Surface等
        ...
        源码总结：内部重新开了一个线程，执行EGL的创建，销毁，使用等各种方法
 *
 */
public class GLEnvironment implements SurfaceHolder.Callback2 {
     ...
     * must call {@link #setRenderer} to register a renderer.
     */
    public GLEnvironment(Context context, AttributeSet attrs) {
        init();
    }
    ...
    private final WeakReference<GLEnvironment> mThisWeakRef =
            new WeakReference<GLEnvironment>(this);
